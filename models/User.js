...
User.prototype.validate=function(){
  return new Promise(async (resolve,reject)=>{
    ...
    // check username is valid check if already exists
    ...
    if(validator.isEmail(this.data.email)){
      ...
      if(emailExists){this.errors.push("That email is already been taken")}
    }
    resolve()
  })
}
    ...
    usersCollection.findOne({username:username}).then(function(userDoc){
      if(userDoc){
        ...
        userDoc={
          ...
          username:userDoc.data.username, avatar:userDoc.avatar
        }
        resolve(userDoc)
      }else{
        reject()
